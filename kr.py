...
# W10 = W9 + W8
# W=W10/(1-W10)
##используя sympy находим коэф упрощенной
...
""" Критерий Найквиста - Критерий Найквиста позволяет судить об устойчивости замкнутой системы по виду КЧХ 
разомкнутой системы. Замкнутая система будет устойчивой, если годограф КЧХ устойчивой разомкнутой системы 
не охватывает точку (-1; j0) комплексной плоскости.Данный критерий позволяет наглядно судить 
о запасе устойчивости по модулю и по фазе.
"""
def nyquis(W):
    plt.figure(1)
    con.nyquist_plot(W)
...
""" Теорема Ляпунова (следствие). Система будет устойчивой, если все корни (полюса)  будут
 иметь отрицательную вещественную часть. Если хотя бы один корень (полюс) будет иметь положительную вещественную часть, 
...
""" Для устойчивой системы управления n-го порядка необходимо и достаточно, 
чтобы годограф Михайлова при изменении w, начиная с вещественной положительной полуоси, 
проходил против часовой стрелки последовательно через n – квадрантов. """
def mihalych(W1):
    P2 = (W1.den[0])
    a = P2[0]  # коэфициенты
    plt.figure(1)
